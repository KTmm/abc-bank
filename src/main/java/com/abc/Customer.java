...
public class Customer {
	private long customerId; // customer id is different for each customer
	private String name;
	private HashMap<Long, Account> accounts;
	
	/**If we have a Database customerId would be generated in database but here I will use manual customerId*/
	...
	public double getTotalInterestEarned(Date date) throws OverDraftException {
	...
	private String statementForAccount(Account a, Date date) throws OverDraftException {
		String s = "";
		s += a.getAccountType().getAccountTypeStatement() + "\n";

		// Now total up all the transactions
		double total = a.getBalanceToDate();
		for (Transaction t : a.getAllTransactionsForAccountToDate(date)) {
